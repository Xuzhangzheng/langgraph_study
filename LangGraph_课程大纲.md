...
**对应文件**：`17_multi_agent_collaboration_graph.py`  
**代码文件记录时间**：2026-05-02 09:05:00  
**Java 对照**：`java/src/main/java/study/langgraph/lessons/l17_multi_agent_collaboration/Lesson17App.java`（与 Python 同拓扑：`normalize_goal` + `planner` / `executor` / `critic` + `finalize_pass` / `finalize_abort`）  
**图导出**：运行脚本后同目录 `17_multi_agent_collaboration_graph.png`（失败时 `17_multi_agent_collaboration_graph.mmd`）。

**具体内容**：**单图三角色**：`Planner` 写 **`plan_outline`**，`Executor` 依计划写 **`draft_answer`**，`Critic` 裁定 **`pass` | `revise_executor` | `revise_planner`** 并写 **`critic_feedback`**（状态即消息协议）；条件边 **`route_after_critic`** 在 **`iteration` > `max_iterations`** 时进入 **`finalize_abort`**，对齐第 4 课「回路须设上限」。`normalize_goal` 处理空目标收口；离线验收关键字 **`FORCE_PASS`**、`FORCE_REVISE_EXEC_ONCE`、`FORCE_REVISE_PLAN_ONCE`、`FORCE_MAX_SPIN`（打转直至中止）。**`mode=llm`** 时按 **`LLM_PROVIDER`** 与第 **6 / 16** 课：`openai` → `ChatOpenAI`；**`ark`** → `volcenginesdkarkruntime` + `responses.create`；解析失败或未配置密钥则节点内回落模板。**Java** 路径下 Ark 仍为占位文案，完整 Ark 以 Python 脚本为准。

**图结构**：

```text
START → normalize_goal ──route────────→ planner → executor → critic ─route──→ finalize_pass → END
                           │                                                      ├→（revise_executor → executor）
                           └→ seal_invalid_goal → END                            ├→（revise_planner → planner）
                                                                                  └→ finalize_abort → END
```

**本课 DoD 提示**：跑通 **`FORCE_*`**、`FORCE_MAX_SPIN + max_iterations` 中止、`user_goal` 空输入；对照 `final_answer`、`critic_verdict`、`diagnostics` 与 Python/Java 状态键；有 Maven 时运行 **`Lesson17App`**。

---

## 第18课：生产化部署与版本治理

**对应文件**：`18_production_governance_graph.py`  
**代码文件记录时间**：2026-05-02 15:46:08  
**Java 对照**：`java/src/main/java/study/langgraph/lessons/l18_production_governance/Lesson18App.java`（与 Python 同拓扑：`normalize_release` + `load_env_profile` + `verify_contract_and_pins` + `preflight_health` + `assess_release_risk` + `governance_finalize` / `seal_blocked` / `seal_invalid_release`）  
**图导出**：运行脚本后同目录 `18_production_governance_graph.png`（失败时 `18_production_governance_graph.mmd`）。

**具体内容**：**发布治理流水线节点化**：`load_env_profile`（`app_env`→profile + `REPO_DEPS_PIN` 快照）→ `verify_contract_and_pins`（**`STATE_CONTRACT_VERSION`** 与 **`EXPECTED_STATE_CONTRACT_VERSION`**；**`FORCE_CONTRACT_FAIL` / `FORCE_LOCK_FAIL`**）→ `preflight_health`（**`FORCE_PREFLIGHT_FAIL`**；**`prod`** 需 **`REQUIRE_TRACING_IN_PROD`**）→ 条件边 **`route_after_gates`** → 通过则 **`assess_release_risk`**（`mode=llm` 时按 **`LLM_PROVIDER`** 与第 **6 / 16 / 17** 课：**`openai`** → `ChatOpenAI`；**`ark`** → `volcenginesdkarkruntime` + `responses.create` + 单串 **`input`；默认 `fallback` 模板）→ **`governance_finalize`**（**`FORCE_HOLD` / `FORCE_ROLLBACK`** 或风险正文关键词 **`致命|严重事故|P0`** → `hold`）；失败收口 **`seal_blocked`**；空 **`release_id`** → **`seal_invalid_release`**。环境变量 **`REPO_DEPS_PIN`** 在 **staging/prod** 必须与 profile 内 **`expected_deps_pin`** 一致。

**图结构**：

```text
START → normalize_release ─route──→ load_env_profile → verify_contract_and_pins → preflight_health ─route──→ assess_release_risk → governance_finalize → END
                          │                                                                                    └→ seal_blocked → END
                          └→ seal_invalid_release → END
```

**本课 DoD 提示**：跑通 **`FORCE_CONTRACT_FAIL` / `FORCE_LOCK_FAIL`**、空 **`release_id`**、**`json-demo` 主路径**；**staging/prod** 补全 **`REPO_DEPS_PIN`** 后再验真实锁定；**prod** 预检需 **`REQUIRE_TRACING_IN_PROD=true`**；对照 **`governance_decision`** 与 **`diagnostics`**；有 Maven 时运行 **`Lesson18App`**。

---

## 第19课：课程项目（Capstone）— 多文件「支持台」应用

**对应交付**：Python **包目录** `lesson19_support_desk/`（非单文件脚本）；入口 `python -m lesson19_support_desk`。  
**代码文件记录时间**：2026-05-02 17:30:00  
**Java 对照**：`java/src/main/java/study/langgraph/lessons/l19_support_desk/`（`L19State` / `L19Routing` / `SupportDeskNodes` / `SupportDeskWorkflow` / `Lesson19App` —— 职责与 Python 分包对齐）  
**图导出**：首次运行 `python -m lesson19_support_desk` 或带 `--export` 时生成 `lesson19_support_desk/lesson19_support_desk_workflow.png`（失败时同目录 `.mmd`）。

**包内职能划分（工程化呈现，而非一个 demo 文件）**：

| 模块 | 职责 |
|------|------|
| `settings.py` | `.env` 启动、`CAPSTONE_LLM_MODE`、`CAPSTONE_MAX_ATTEMPTS` |
| `state.py` | `SupportDeskState` 契约（接口不变清单唯一源） |
| `tools_runtime.py` | 纯函数工具（安全算术、本地时间） |
| `llm_client.py` | 与第 6 课一致的 OpenAI / Ark 网关 |
| `routing.py` | 无副作用的条件边纯函数（可单测） |
| `node_ingest.py` / `node_classify.py` / `node_tools.py` / `node_generate.py` / `node_evaluate.py` / `node_finalize.py` | 各节点实现 |
| `workflow.py` | **仅** `StateGraph` 接线与导出图 |
| `application.py` | `SupportDeskApplication`：`compile(checkpointer=InMemorySaver())`、统一 `invoke` |
| `regression.py` | 黄金套件黑盒回归（`--regression` 非零退出模拟 CI） |
| `__main__.py` | CLI：`--regression` / `--export` / 默认 demo |

**具体内容**：售前 **ingest → classify →（工具算术/时间 | 带评估回路的生成）→ finalize**；综合第 **4** 课（`attempt`/`max_attempts`）、**8** 课（工具路由）、**6** 课（LLM/Ark）、**12** 课（checkpointer + `thread_id`）、**15** 课（离线套件思想）。默认 `mode=fallback` 零密钥；`CAPSTONE_LLM_MODE=llm` 且配置 `LLM_PROVIDER` + 密钥可走真实模型。

**图结构**：

```text
START → ingest ─route──→ classify_intent ─route──→ tool_calculator ──┐
                          │                         tool_time ────────┼──→ finalize_reply → END
                          │                         generate_reply → evaluate_reply ─route──→ finalize_reply → END
                          │                                                            └→ generate_reply（循环）
                          └→ finalize_reply（invalid 入参）──────────────────────────────────────────→ END
```

**本课 DoD 提示**：`python -m lesson19_support_desk --regression` 全绿；对照包内文件职责；可选 `mvn -f java/pom.xml exec:java -Dexec.mainClass=study.langgraph.lessons.l19_support_desk.Lesson19App`（需本机 JDK17+Maven）；JavaScript 引擎不可用时算术节点可能回落失败，与 Python AST 路径非 100% 一致属预期。

---

## 第20课：复盘与进阶路线

**对应交付**：Python 包目录 **`lesson20_course_review/`**；入口 `python -m lesson20_course_review`。  
**代码文件记录时间**：2026-05-02 18:25:00  
**Java 对照**：`java/src/main/java/study/langgraph/lessons/l20_course_review/`（`CourseCatalog` / `AdvancementRoadmap` / `Lesson20App`，artifact 存在性巡检；**完整回归以 Python `--verify` 为准**）  
**附属产出**：`python -m lesson20_course_review --mmd` → 仓库根目录 **`lesson20_learning_path.mmd`**（学习路径 Mermaid，与 `catalog` 同源）。

**包内职能**：

| 模块 | 职责 |
|------|------|
| `catalog.py` | 第 **1～19** 课元数据表（主文件 / 关键词），单一数据源 |
| `advancement.py` | **约 30 天 / 60～90 天** 进阶备忘（工程向） |
| `mermaid_path.py` | 从 `catalog` 生成 `flowchart LR` 学习路径 |
| `verify.py` | **artifact 巡检** + 挂载 **`lesson19_support_desk.regression.run_suite`** |
| `__main__.py` | 默认打印课表与进阶；`--mmd` / `--verify` |

**具体内容**：**无可单独业务意义的「新 Agent 图」**——本课产品是 **复盘工具**：对照大纲完成度、导出脑图、跑结课级验证。`--verify` 任一项失败时 **exit code 1**（模拟结课 CI）。**第 13 课 streaming 的完整补遗见第 21 课（选修）**。

**图结构**：无固定运行时 LangGraph；`lesson20_learning_path.mmd` 为 **学习路径**（课号依赖图），非执行拓扑。

**本课 DoD 提示**：`python -m lesson20_course_review` 可读；`--mmd` 生成 `.mmd`；`--verify` 全绿； milestone E 自检完成。
...
- 已完成：第 1 课～第 **20** 课（第 **20** 课为 **`lesson20_course_review/`** 复盘工具包 + Mermaid 学习路径 + `--verify`）
- 建议插学（可选）：**专题 A+ Reducer**（`04b_reducer_graph.py`，与第 5 课前/第 10 课并行搭配）
- **选修（延伸）**：**第 21 课** `stream_mode` 补遗（大纲已写分工；示例脚本仍规划中）
- **主路径已闭合**：第 **1～20** 课；后续以第 21 课选修与官方文档为增量

**DoD 自检（第 1–20 课）**：每课 Python 可运行；第 8 课起建议对照 Java（第 12 课无 Java，第 **13～20** 课有 Java；**14b** 亦有 Java）；第 9 课主图嵌套子图；第 10 课列表合并 + 单点 `aggregate`；第 11 课 `thread_id` 与 `resume`；第 12 课 `thread_id` 与 `get_state` / `history`；第 13 课 `stream` 多 mode + 节点日志关联；第 14 课 `risk_status` 三元路由 + `backoff_then_retry` 环；**14b** 对照支付域语义与 14 课抽象版同构；第 **15** 课 SUT + 黄金套件 + `MIN_PASS_RATIO` 门禁 + 评测编排图与 SUT **双 PNG 导出**；第 **16** 课 RAG 条件边（无命中 / 非法输入）+ `citations` 与 chunk 列表契约 + 图 PNG 导出；第 **17** 课 `normalize_goal`、`critic_verdict`、修订回路、`finalize_abort` 与 **`FORCE_*` 离线桩**；第 **18** 课 `contract_check` / `dependency_lock_check` / `preflight_check`、`seal_blocked`、`STATE_CONTRACT_VERSION` 与 **`REPO_DEPS_PIN`** 及 Ark/OpenAI 风险评估路径；第 **19** 课 **`lesson19_support_desk`** 包内分层、`SupportDeskApplication` + **`InMemorySaver`**、回归与 **LLM-as-judge 质检**（`mode=llm`）；第 **20** 课 **`lesson20_course_review`**、`--mmd` 学习路径、**`--verify`** 结课巡检。
